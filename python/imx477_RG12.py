...
camera = v4l2.open('/dev/video'+camera_num, width, height, 'RG12')
...
v4l2.set_control(camera, digital_gain, 800)
...
while True:
    ...
    width = int(len(data)/2/height)

    image_array = np.frombuffer(data, dtype=np.uint16)
    image_array = image_array >> 4
    image_array = np.array(image_array, dtype=np.uint8)

    # image = image_array.reshape(int(height), int(width))

    # # image = cv2.demosaicing(image, cv2.COLOR_BayerBG2BGR)
    # image = cv2.cvtColor(image, cv2.COLOR_BayerBG2BGR)
    # # image = cv2.cvtColor(image, cv2.COLOR_BayerBG2GRAY)

    # image = image_array.reshape(int(height), int(width))
    image = image_array.reshape(int(height*width/4), 4)
    ...
    image = cv2.resize(image, (int(width*0.36), int(height*0.36)))

    fps = v4l2.get_fps(camera)

    image = cv2.putText(image, 'FPS:'+str(fps), (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100, 100, 250), 1)
